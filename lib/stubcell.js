...
StubCell.prototype._getMatchEntry = function(req, entries){
  ...
  for(var i = 0, l = entries.length; i < l; i++){
    ...
    if (request.method === "ALL" ? true :
       request.method.toLowerCase() === req.method.toLowerCase() &&
       check.url(request.pathRegexp, parsedUrl.pathname) &&
       check.headers(request.headers, req.headers, this.looseCompare) &&
       check.query(request.query, req.query, this.looseCompare) &&
       check.body(request.body, req.body, this.looseCompare)
      )
    {
     return entry;
    }
