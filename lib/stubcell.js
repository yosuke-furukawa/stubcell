...
StubCell.prototype.server = function() {
  ...
  if (Object.keys(this.record).length > 0) {
    app.use(function(req, res, next) {
      var record = this.record === true ? {} : this.record;
      var reqBody = req.body || {};
      var isJSONRPC = reqBody.jsonrpc;
      var ignoreJson = true;
      var filepath = this.fileFromRequest(req.url, "", false);
      var storePath = isJSONRPC ? filepath + "/" + reqBody.method + ".json" : filepath + ".json";
...
StubCell.prototype.fileFromRequest = function(requestUrl, filepath, appendJson) {
  ...
  app[method](requestUrl, function(req, res, next) {
    ...
    if (isJSONRPC) {
      ...
      if(entry.request.body) {
        var match = check.body(entryBody, reqBody);
        if (!match) return next(); 
      } 
      if(entry.request.query) {
        var match = check.query(entryQuery, reqQuery);
        if (!match) return next(); 
      }
    }
    //response header
    ...
    fs.readFile(file, function(err, data) {
      ...
      if (this.debug) {
        ...
        console.log("\033[36m" + "[return jsonfile] = " + file +"\033[39m");
      }
      res.send(data);
    }.bind(this));
  }.bind(this));
